...
- **Авто-ответы**: при входящем DM генерирует ответ через OpenAI с
  контекстом (критерии + Obsidian-нотатки + последние 10 сообщений
  переписки). Если отправитель ещё не в БД — модуль сам создаёт запись
  в `clients` (см. `auto_register`) и продолжает диалог.
...
| `auto_register`   | `true` — отвечать всем подряд и записывать новых в `clients` (без критериев → проактивные рассылки им не идут) |
...
С `auto_register = true` (по умолчанию) модуль сам заводит запись в
`clients` для каждого нового собеседника при первом DM:

- `name` берётся из Telegram first/last name (или `@username`,
  fallback `tg:<id>`).
- `username`, `telegram_id` сохраняются.
- `criteria = {}`, `initiate = false` — без критериев проактивные
  рассылки квартир ему не идут, только AI-ответы на входящие.
- `status = active`, `auto_enabled = true` — отвечает сразу.

Когда выяснишь критерии (бюджет, район, комнаты) — заполни их через
Supabase Dashboard или CRM-фронтенд, и клиент попадёт в проактивную
рассылку (`initiate = true`).

Можно и руками с самого начала — Supabase Dashboard → Table Editor →
`clients`. Минимально:
